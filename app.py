...
def get_activity():
    ...
    notifications = db_execute('SELECT notifications.*, sender.username AS sender, reviews.artist, reviews.song_title, reviews.cover_img_url FROM notifications JOIN users AS sender ON sender.id = notifications.sender_id JOIN reviews ON reviews.id = notifications.review_id WHERE recipient_id=%(user_id)s ORDER BY notifications.created_at DESC', user_id=user_id)
